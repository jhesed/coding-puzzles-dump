...
elements to the left. This also applies to
...
def get_pivot_index(nums: List[int]) -> int:
    ...
    while left_index < len(nums):
        ...
        if left_index > 0:
            left_value += nums[left_index - 1]

        right_value = sum_value - left_value - nums[left_index]
...
def get_pivot_index_slow(nums: List[int]) -> int:
    """
    Slow version (9s)
    """
    ...
    for index, num in enumerate(nums):
        ...
        if index == 0:
            ...
            left_value = sum(nums[:index])

        # Compute right value
        if index == list_length:
            right_value = 0
        else:
            right_value = sum(nums[index + 1 :])
